import { Link } from "react-router-dom";
import "./header.css";
export default function Header() {
  return (
    <div className='head'>
    <div className="headLeft ">
    <i className=" headIcon fa-brands fa-facebook"></i>
    <i className=" headIcon fa-brands fa-twitter"></i>
    <i className=" headIcon fa-brands fa-instagram"></i>
    <i className=" headIcon fa-brands fa-pinterest"></i>
    </div>
    <div className="headCenter">
      <ul className="headList">
        <Link to="/" className="headListItem text-decoration-none">Home</Link>
        <Link to="/blogs" className="headListItem text-decoration-none">Blogs</Link>
        {/* <li className="headListItem text-decoration-none">Contact</li> */}
        <Link to="/write" className="headListItem">Write Blogs</Link>
        {/* <li className="headListItem">Logout</li> */}


      </ul>
    </div>
    <div className="headRight">
     <img className="headImage" src="data:image/jpeg;base64," ></img>
    <i className=" searchIcon fa-sharp fa-solid fa-magnifying-glass"></i>
    </div>
  </div>
  )
}
